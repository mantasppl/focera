...
import JpgToGifDropzone from "@/components/tools/JpgToGifDropzone";
import { formatFileSize } from "@/lib/image";
import {
  convertJpgToGif,
  describeJpgGifOutput,
  downloadConvertedGif,
  downloadJpgGifResult,
  JPG_TO_GIF_DELAY_PRESETS,
  JPG_TO_GIF_QUALITY_PRESETS,
  JPG_TO_GIF_SIZE_PRESETS,
  MAX_JPG_FILES,
  revokeJpgToGifResult,
  type JpgToGifDelay,
  type JpgToGifMode,
  type JpgToGifQuality,
  type JpgToGifResult,
  type JpgToGifSize,
} from "@/lib/jpg-to-gif";
...
export default function JpgToGif() {
  ...
  const resultRef = useRef<JpgToGifResult | null>(null);

  const [entries, setEntries] = useState<ImageEntry[]>([]);
  const [mode, setMode] = useState<JpgToGifMode>("animated");
  const [size, setSize] = useState<JpgToGifSize>("medium");
  const [quality, setQuality] = useState<JpgToGifQuality>("medium");
  const [delay, setDelay] = useState<JpgToGifDelay>(200);
  const [loading, setLoading] = useState(false);
  const [progressText, setProgressText] = useState("");
  const [error, setError] = useState("");
  const [result, setResult] = useState<JpgToGifResult | null>(null);
  ...
  useEffect(() => {
    return () => {
      ...
      revokeJpgToGifResult(resultRef.current);
    };
  }, []);

  function clearResult() {
    setResult((current) => {
      revokeJpgToGifResult(current);
      return null;
    });
    setSelectedId(null);
  }
  ...
  async function handleConvert() {
    ...
    try {
      const converted = await convertJpgToGif(files, {
        mode,
        size,
        quality,
        delay,
      ...
      if (controller.signal.aborted) {
        revokeJpgToGifResult(converted);
        return;
      }

      setResult(converted);
      setSelectedId(converted.images[0]?.id ?? null);
      await downloadJpgGifResult(converted, files);
      setProgressText("");
      trackSuccess();
    } catch (err) {
  ...
  async function handleDownloadAgain() {
    ...
    setError("");
    try {
      await downloadJpgGifResult(result, files);
    } catch {
      ...
      <div className="tool-panel">
        ...
        <div className="heic-to-jpg__options">
          ...
          <div className="ui-field">
            ...
            >
              {JPG_TO_GIF_SIZE_PRESETS.map((preset) => {
          ...
          <div className="ui-field">
            ...
            >
              {JPG_TO_GIF_QUALITY_PRESETS.map((preset) => {
          ...
          {showAnimatedOptions ? (
            <div className="ui-field">
              ...
              >
                {JPG_TO_GIF_DELAY_PRESETS.map((preset) => {
      ...
      <div className="tool-panel tool-panel--preview">
          ...
          {loading ? (
              ...
              <ul className="png-to-pdf__stats" aria-label="Conversion summary">
                <li>
                  <span className="png-to-pdf__stat-label">
                    {result.animated ? "Frames" : "JPG files"}
                  </span>
            ...
            <div className="png-to-pdf__empty">
              <p className="tool-placeholder">
                {fileCount === 0
                  ? "Upload a JPG image to convert it to GIF"
                  : fileCount === 1
                    ? "1 JPG file queued · click Convert to GIF"
                    : mode === "animated"
                      ? `${fileCount} JPG files queued · click Convert to GIF for one animation`
                      : `${fileCount} JPG files queued · click Convert to GIF`}
              </p>
              {fileCount > 0 ? (
        ...
        <p className="tool-hint">
          {hasResult
            ...
            : "JPG to GIF runs in your browser · files never upload to Focera"}
        </p>
      </div>
    </div>
  );
