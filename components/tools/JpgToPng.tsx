...
import JpgToPngDropzone from "@/components/tools/JpgToPngDropzone";
import { formatFileSize } from "@/lib/image";
import {
  convertJpgToPng,
  describeJpgPngOutput,
  downloadConvertedPng,
  downloadJpgPngResult,
  MAX_JPG_FILES,
  revokeJpgToPngResult,
  type JpgToPngResult,
} from "@/lib/jpg-to-png";
...
export default function JpgToPng() {
  ...
  const resultRef = useRef<JpgToPngResult | null>(null);
  ...
  useEffect(() => {
    return () => {
      ...
      revokeJpgToPngResult(resultRef.current);
    };
  }, []);

  function clearResult() {
    setResult((current) => {
      revokeJpgToPngResult(current);
      return null;
    });
    setSelectedId(null);
  }
  ...
  async function handleConvert() {
    ...
    try {
      const converted = await convertJpgToPng(files, {
      ...
      if (controller.signal.aborted) {
        revokeJpgToPngResult(converted);
        return;
      }

      setResult(converted);
      setSelectedId(converted.images[0]?.id ?? null);
      await downloadJpgPngResult(converted, files);
      setProgressText("");
      trackSuccess();
    } catch (err) {
  ...
  async function handleDownloadAgain() {
    ...
    setError("");
    try {
      await downloadJpgPngResult(result, files);
    } catch {
      ...
      <div className="tool-panel tool-panel--preview">
          ...
          {loading ? (
            ...
            <div className="png-to-pdf__empty">
              <p className="tool-placeholder">
                {fileCount === 0
                  ? "Upload a JPG image to convert it to PNG"
                  : `${fileCount} JPG file${fileCount === 1 ? "" : "s"} queued · click Convert to PNG`}
              </p>
              {fileCount > 0 ? (
        ...
        <p className="tool-hint">
          {hasResult
            ...
            : "JPG to PNG runs in your browser · files never upload to Focera"}
        </p>
      </div>
    </div>
  );
