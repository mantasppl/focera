...
export default function HeicToPng() {
  ...
  const resultRef = useRef<HeicToPngResult | null>(null);

  const [entries, setEntries] = useState<ImageEntry[]>([]);
  const [loading, setLoading] = useState(false);
  const [progressText, setProgressText] = useState("");
  const [error, setError] = useState("");
  const [result, setResult] = useState<HeicToPngResult | null>(null);
  ...
  useEffect(() => {
    return () => {
      ...
      revokeHeicToPngResult(resultRef.current);
    };
  }, []);

  function clearResult() {
    setResult((current) => {
      revokeHeicToPngResult(current);
      return null;
    });
    setSelectedId(null);
  }
  ...
  async function handleConvert() {
    ...
    try {
      const converted = await convertHeicToPng(files, {
      ...
      if (controller.signal.aborted) {
        revokeHeicToPngResult(converted);
        return;
      }
  ...
  async function handleDownloadZip() {
    ...
    setError("");
    try {
      await downloadHeicPngResult(result, files);
    } catch {
