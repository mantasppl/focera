...
import PngToSvgDropzone from "@/components/tools/PngToSvgDropzone";
...
import {
  convertPngToSvg,
  describePngSvgOutput,
  detailLabel,
  downloadPngSvgResult,
  MAX_PNG_FILES,
  revokePngToSvgResult,
  type PngSvgDetail,
  type PngToSvgResult,
} from "@/lib/png-to-svg";
...
const DETAIL_OPTIONS: {
  ...
  const resultRef = useRef<PngToSvgResult | null>(null);

  const [detail, setDetail] = useState<PngSvgDetail>("balanced");
  const [entries, setEntries] = useState<ImageEntry[]>([]);
  const [loading, setLoading] = useState(false);
  const [progressText, setProgressText] = useState("");
  const [error, setError] = useState("");
  const [result, setResult] = useState<PngToSvgResult | null>(null);
  ...
  useEffect(() => {
    return () => {
      ...
      revokePngToSvgResult(resultRef.current);
    };
  }, []);

  function clearResult() {
    setResult((current) => {
      revokePngToSvgResult(current);
      return null;
    });
    setSelectedId(null);
  }
  ...
  async function handleConvert() {
    ...
    try {
      const converted = await convertPngToSvg(files, {
        detail,
      ...
      if (controller.signal.aborted) {
        revokePngToSvgResult(converted);
        return;
      }
  ...
  async function handleDownloadZip() {
    ...
    setError("");
    try {
      await downloadPngSvgResult(result, files);
    } catch {
  ...
  return (
    <>
      <ImageEditorShell
        ...
        loading={loading}
        loadingText={progressText || "Tracing PNG…"}
        loadingSubtext="Vectorization runs locally in your browser."
        previewTitle="Preview"
        previewMeta={
          hasResult && result
            ? `${describePngSvgOutput(result)} · {detailLabel(result.detail)}`
            : hasSource
              ? `${fileCount} file${fileCount === 1 ? "" : "s"} queued`
              : "Upload images to start"
        }
        previewHint={
          hasSource && !hasResult ? "Click Convert to SVG" : undefined
        }
        privacyHint={
          hasResult
            ? "Processed locally on your device"
            : "PNG to PNG runs in your browser · files never upload to Focera"
        }
        sidebar={
          <>
            <PngToSvgDropzone
        ...
        {hasResult && result && activeImage ? (
          ...
          <div className="png-to-pdf__empty">
            <p className="tool-placeholder">
              {`${fileCount} PNG file${fileCount === 1 ? "" : "s"} queued · click Convert to SVG`}
            </p>
