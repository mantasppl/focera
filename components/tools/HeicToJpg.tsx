...
import HeicToJpgDropzone from "@/components/tools/HeicToJpgDropzone";
import { formatFileSize } from "@/lib/image";
import {
  convertHeicToJpg,
  describeHeicJpgOutput,
  downloadConvertedJpg,
  downloadHeicJpgResult,
  MAX_HEIC_FILES,
  qualityLabel,
  revokeHeicToJpgResult,
  type HeicJpgQuality,
  type HeicToJpgResult,
...
export default function HeicToJpg() {
  ...
  const resultRef = useRef<HeicToJpgResult | null>(null);

  const [entries, setEntries] = useState<ImageEntry[]>([]);
  const [quality, setQuality] = useState<HeicJpgQuality>(0.85);
  const [loading, setLoading] = useState(false);
  const [progressText, setProgressText] = useState("");
  const [error, setError] = useState("");
  const [result, setResult] = useState<HeicToJpgResult | null>(null);
  ...
  useEffect(() => {
    return () => {
      ...
      revokeHeicToJpgResult(resultRef.current);
    };
  }, []);

  function clearResult() {
    setResult((current) => {
      revokeHeicToJpgResult(current);
      return null;
    });
    setSelectedId(null);
  }
  ...
  async function handleConvert() {
    if (fileCount === 0) {
      setError("Upload at least one HEIC (.heic or .heif) to get started.");
      return;
    }
    ...
    setLoading(true);
    setError("");
    setProgressText("Loading HEIC decoder…");
    clearResult();

    try {
      const converted = await convertHeicToJpg(files, {
        quality,
      ...
      if (controller.signal.aborted) {
        revokeHeicToJpgResult(converted);
        return;
      }

      setResult(converted);
      setSelectedId(converted.images[0]?.id ?? null);
      await downloadHeicJpgResult(converted, files);
      setProgressText("");
      trackSuccess();
    } catch (err) {
      ...
      const message =
        err instanceof Error
          ? err.message
          : "Could not convert these HEIC files. Try fewer or smaller photos.";
      setError(message);
      setProgressText("");
    } finally {
  ...
  async function handleDownloadAgain() {
    ...
    setError("");
    try {
      await downloadHeicJpgResult(result, files);
    } catch {
      ...
      <div className="tool-panel tool-panel--preview">
          ...
          {loading ? (
            ...
            <div className="png-to-pdf__success">
              ...
              <p className="png-to-pdf__success-meta">
                {describeHeicJpgOutput(result)} · {qualityLabel(result.quality)}
              </p>
            ...
            <div className="png-to-pdf__empty">
              <p className="tool-placeholder">
                {fileCount === 0
                  ? "Upload a HEIC photo to convert it to JPG"
                  : `${fileCount} HEIC file${fileCount === 1 ? "" : "s"} queued · click Convert to JPG`}
              </p>
              {fileCount > 0 ? (
        ...
        <p className="tool-hint">
          {hasResult
            ...
            : "HEIC to JPG runs in your browser · files never upload to Focera"}
        </p>
      </div>
    </div>
  );
