...
import JpgToWebpDropzone from "@/components/tools/JpgToWebpDropzone";
import { formatFileSize } from "@/lib/image";
import {
  convertJpgToWebp,
  describeJpgWebpOutput,
  downloadConvertedWebp,
  downloadJpgWebpResult,
  MAX_JPG_FILES,
  qualityLabel,
  revokeJpgToWebpResult,
  type JpgToWebpResult,
  type JpgWebpQuality,
...
export default function JpgToWebp() {
  ...
  const resultRef = useRef<JpgToWebpResult | null>(null);

  const [entries, setEntries] = useState<ImageEntry[]>([]);
  const [quality, setQuality] = useState<JpgWebpQuality>(0.85);
  const [loading, setLoading] = useState(false);
  const [progressText, setProgressText] = useState("");
  const [error, setError] = useState("");
  const [result, setResult] = useState<JpgToWebpResult | null>(null);
  ...
  useEffect(() => {
    return () => {
      ...
      revokeJpgToWebpResult(resultRef.current);
    };
  }, []);

  function clearResult() {
    setResult((current) => {
      revokeJpgToWebpResult(current);
      return null;
    });
    setSelectedId(null);
  }
  ...
  async function handleConvert() {
    ...
    try {
      const converted = await convertJpgToWebp(files, {
        quality,
      ...
      if (controller.signal.aborted) {
        revokeJpgToWebpResult(converted);
        return;
      }

      setResult(converted);
      setSelectedId(converted.images[0]?.id ?? null);
      await downloadJpgWebpResult(converted, files);
      setProgressText("");
      trackSuccess();
    } catch (err) {
  ...
  async function handleDownloadAgain() {
    ...
    setError("");
    try {
      await downloadJpgWebpResult(result, files);
    } catch {
      ...
      <div className="tool-panel tool-panel--preview">
          ...
          {loading ? (
            ...
            <div className="png-to-pdf__success">
              ...
              <p className="png-to-pdf__success-meta">
                {describeJpgWebpOutput(result)} · {qualityLabel(result.quality)}
              </p>
            ...
            <div className="png-to-pdf__empty">
              <p className="tool-placeholder">
                {fileCount === 0
                  ? "Upload a JPG image to convert it to WebP"
                  : `${fileCount} JPG file${fileCount === 1 ? "" : "s"} queued · click Convert to WebP`}
              </p>
              {fileCount > 0 ? (
        ...
        <p className="tool-hint">
          {hasResult
            ...
            : "JPG to WebP runs in your browser · files never upload to Focera"}
        </p>
      </div>
    </div>
  );
