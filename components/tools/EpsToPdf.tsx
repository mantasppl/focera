...
import EpsToPdfDropzone from "@/components/tools/EpsToPdfDropzone";
import { formatFileSize } from "@/lib/image";
import {
  convertEpsToPdf,
  describeEpsPdfOutput,
  downloadEpsPdf,
  pageSizeLabel,
  revokeEpsToPdfResult,
  type EpsPdfPageSize,
  type EpsToPdfResult,
} from "@/lib/eps-to-pdf";
...
export default function EpsToPdf() {
  ...
  const resultRef = useRef<EpsToPdfResult | null>(null);

  const [sourceFile, setSourceFile] = useState<File | null>(null);
  const [pageSize, setPageSize] = useState<EpsPdfPageSize>("crop");
  const [loading, setLoading] = useState(false);
  const [progressText, setProgressText] = useState("");
  const [error, setError] = useState("");
  const [result, setResult] = useState<EpsToPdfResult | null>(null);
  ...
  useEffect(() => {
    return () => {
      ...
      revokeEpsToPdfResult(resultRef.current);
    };
  }, []);

  function clearResult() {
    setResult((current) => {
      revokeEpsToPdfResult(current);
      return null;
    });
  }
  ...
  async function handleConvert() {
    ...
    try {
      const converted = await convertEpsToPdf(sourceFile, {
        pageSize,
      ...
      if (controller.signal.aborted) {
        revokeEpsToPdfResult(converted);
        return;
      }
  ...
  function handleDownloadAgain() {
    ...
    downloadEpsPdf(result.blob, sourceFile);
  }
          ...
          {loading ? (
            ...
            <div className="word-to-pdf__success">
              ...
              <ul className="word-to-pdf__stats" aria-label="Conversion summary">
                ...
                <li>
                  <span className="word-to-pdf__stat-label">Original</span>
                  <span className="word-to-pdf__stat-value">
            ...
            <p className="tool-placeholder">
              Upload an EPS file and convert it to PDF here
            </p>
          )}
        </div>
