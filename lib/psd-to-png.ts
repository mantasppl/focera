...
export type PsdPngBackground = "transparent" | "white";

export type ConvertedPsdPng = {
  id: string;
  sourceName: string;
  blob: Blob;
  url: string;
...
export type PsdToPngResult = {
  images: ConvertedPsdPng[];
  background: PsdPngBackground;
  originalSize: number;
  outputSize: number;
};

export type ConvertPsdToPngOptions = {
  background?: PsdPngBackground;
...
export function backgroundLabel(background: PsdPngBackground): string {
...
async function convertSinglePsd(
  ...
  if (blob.size < 1) {
    throw new Error(`"${file.name}" produced an empty PNG. Try another PSD.`);
  }
...
export async function convertPsdToPng(
  files: File[],
  options: ConvertPsdToPngOptions = {},
): Promise<PsdToPngResult> {
  if (files.length === 0) {
...
export function revokePsdToPngResult(result: PsdToPngResult | null) {
  if (!result) return;
...
export function downloadConvertedPng(image: ConvertedPsdPng) {
  downloadBlob(image.blob, pngFileName(image.sourceName));
}

export async function downloadPsdPngResult(
  result: PsdToPngResult,
  sourceFiles: File[],
) {
...
export function describePsdPngOutput(result: PsdToPngResult): string {
  const count =
    result.images.length === 1 ? "1 PNG" : `${result.images.length} PNGs`;
