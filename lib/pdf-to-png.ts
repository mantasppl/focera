...
export async function convertPdfToPngPages(
  file: File,
  ...
  try {
    for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
      ...
      // White background so transparent PDF areas look correct in PNG viewers
...
export function downloadPagePng(page: ConvertedPage, baseName: string) {
  const safeBase = baseName || "page";
  downloadBlob(page.blob, `${safeBase}-page-${page.pageNumber}.png`);
}
...
export async function downloadAllPagesZip(
  ...
  for (const page of pages) {
    zip.file(`${safeBase}-page-${page.pageNumber}.png`, page.blob);
  }

  const zipBlob = await zip.generateAsync({ type: "blob" });
  downloadBlob(zipBlob, `${safeBase}-png.zip`);
}
