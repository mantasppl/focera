...
export type JpgToGifSize = "small" | "medium" | "large";
export type JpgToGifQuality = "low" | "medium" | "high";
export type JpgToGifDelay = 100 | 200 | 500 | 1000;
export type JpgToGifMode = "separate" | "animated";

export type JpgToGifSizePreset = {
  id: JpgToGifSize;
  label: string;
  hint: string;
  maxDimension: number;
...
export type JpgToGifQualityPreset = {
  id: JpgToGifQuality;
  label: string;
  hint: string;
  maxColors: number;
...
export type JpgToGifDelayPreset = {
  id: JpgToGifDelay;
  label: string;
  hint: string;
};

export const JPG_TO_GIF_SIZE_PRESETS: JpgToGifSizePreset[] = [
...
export const JPG_TO_GIF_QUALITY_PRESETS: JpgToGifQualityPreset[] = [
...
export const JPG_TO_GIF_DELAY_PRESETS: JpgToGifDelayPreset[] = [
...
export type ConvertJpgToGifOptions = {
  mode?: JpgToGifMode;
  size?: JpgToGifSize;
  quality?: JpgToGifQuality;
  delay?: JpgToGifDelay;
...
function getSizePreset(size: JpgToGifSize): JpgToGifSizePreset {
  return (
    JPG_TO_GIF_SIZE_PRESETS.find((preset) => preset.id === size) ??
    JPG_TO_GIF_SIZE_PRESETS[1]
  );
}

function getQualityPreset(quality: JpgToGifQuality): JpgToGifQualityPreset {
  return (
    JPG_TO_GIF_QUALITY_PRESETS.find((preset) => preset.id === quality) ??
    JPG_TO_GIF_QUALITY_PRESETS[1]
  );
}
...
async function convertAnimatedGif(
  ...
  try {
    ...
    const sourceName =
      files.length === 1
        ? files[0].name
        : `${fileBaseName(files[0]) || "jpg"}-animated.jpg`;
...
export async function convertJpgToGif(
  files: File[],
  options: ConvertJpgToGifOptions = {},
): Promise<JpgToGifResult> {
  if (files.length === 0) {
  ...
  for (let index = 0; index < files.length; index += 1) {
    ...
    try {
      const converted = await convertSingleJpg(
        file,
        sizePreset.maxDimension,
        qualityPreset.maxColors,
        options.signal,
...
export function revokeJpgToGifResult(result: JpgToGifResult | null) {
  if (!result) return;
...
export async function downloadJpgGifResult(
  result: JpgToGifResult,
  sourceFiles: File[],
) {
...
export function describeJpgGifOutput(result: JpgToGifResult): string {
  if (result.animated) {
    const frames =
