...
export type ConvertPdfToMobiOptions = {
  mode?: PdfToMobiMode;
...
async function canvasToMobiJpeg(canvas: HTMLCanvasElement): Promise<Uint8Array> {
  let quality = 0.82;
  let data = await canvasToJpegBytes(canvas, quality);

  while (data.length > MAX_MOBI_IMAGE_BYTES && quality > 0.35) {
    quality -= 0.08;
    data = await canvasToJpegBytes(canvas, quality);
  }

  if (data.length <= MAX_MOBI_IMAGE_BYTES) {
    return data;
  }

  // Scale down if quality alone cannot meet the legacy MOBI image limit.
  const scale = Math.sqrt(MAX_MOBI_IMAGE_BYTES / data.length) * 0.92;
...
async function convertTextMode(
...
): Promise<{ html: string; fullText: string; chapterCount: number }> {
  ...
  return {
    html: buildMobiHtml(bookTitle, chapters),
...
async function convertVisualMode(
  ...
  for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
    ...
    const data = await canvasToMobiJpeg(canvas);
    images.push({ data });

    const title = `Page ${pageNumber}`;
    const recindex = formatMobiRecindex(pageNumber);
    chapters.push({
      title,
      bodyHtml: `<p align="center"><img recindex="${recindex}"/></p>`,
    });
  ...
  return {
    html: buildMobiHtml(bookTitle, chapters),
    fullText: `Visual conversion · ${pdf.numPages} page image${pdf.numPages === 1 ? "" : "s"} embedded in MOBI`,
    chapterCount: chapters.length,
    images,
  };
}
...
export async function convertPdfToMobi(
  ...
  try {
    ...
    if (mode === "visual") {
      const converted = await convertVisualMode(pdf, bookTitle, options);
      html = converted.html;
      fullText = converted.fullText;
      chapterCount = converted.chapterCount;
      images = converted.images;
    } else {
      const converted = await convertTextMode(pdf, bookTitle, options);
      html = converted.html;
      fullText = converted.fullText;
      chapterCount = converted.chapterCount;
    }
    ...
    throwIfAborted(options.signal);
    options.onProgress?.(pdf.numPages, pdf.numPages, "Building MOBI");

    const blob = buildMobiBlob({
      title: bookTitle,
      author: "Focera PDF to MOBI",
      html,
      images,
    });
...
export function revokePdfToMobiResult(result: PdfToMobiResult | null) {
  if (result?.url) {
    URL.revokeObjectURL(result.url);
  }
}

export function downloadMobiFile(blob: Blob, sourceFile: File) {
  downloadBlob(blob, `${fileBaseName(sourceFile)}.mobi`);
}

export function describeOutput(result: PdfToMobiResult): string {
