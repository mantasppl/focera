...
export function publicErrorMessage(
  ...
  if (
    /api[_-]?key|secret|token|password|authorization|groq|pollinations|replicate|resend|stack|ECONN|ENOENT|sqlite|libsql|puppeteer|chromium|chrome-headless/i.test(
      message,
    )
  ) {
    return fallback;
