...
export type JpgToSvgResult = {
  images: ConvertedJpgSvg[];
  detail: JpgSvgDetail;
  originalSize: number;
  outputSize: number;
};

export type ConvertJpgToSvgOptions = {
  detail?: JpgSvgDetail;
...
export function detailLabel(detail: JpgSvgDetail): string {
  switch (detail) {
    case "simple":
      return "Simple";
    case "detailed":
...
export async function convertJpgToSvg(
  files: File[],
  options: ConvertJpgToSvgOptions = {},
): Promise<JpgToSvgResult> {
  if (files.length === 0) {
  ...
  for (let index = 0; index < files.length; index += 1) {
    ...
    try {
      const converted = await convertSingleJpg(file, detail, options.signal);
      images.push(converted);
    } catch (err) {
      revokeJpgToSvgResult({ images, detail, originalSize: 0, outputSize: 0 });
      throw err;
    }
  }
...
export function revokeJpgToSvgResult(result: JpgToSvgResult | null) {
  if (!result) return;
...
export function downloadConvertedSvg(image: ConvertedJpgSvg) {
  downloadBlob(image.blob, svgFileName(image.sourceName));
}

export async function downloadJpgSvgResult(
  result: JpgToSvgResult,
  sourceFiles: File[],
) {
...
export function describeJpgSvgOutput(result: JpgToSvgResult): string {
  const count =
